package com.leadmaster.service.validator;

import java.util.Arrays;
import java.util.List;
import java.util.regex.Pattern;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.validation.Errors;
import org.springframework.validation.Validator;

import com.leadmaster.common.contant.Constant;
import com.leadmaster.common.dto.CustomerTypeDTO;
import com.leadmaster.common.utils.DateUtils;
import com.leadmaster.common.utils.UserUtils;
import com.leadmaster.common.validator.CustomValidator;

public class CustomerTypeValidator implements Validator {

	private static final String BAD_REQUEST_ERROR_CD = Constant.BAD_REQUEST_ERROR_CD;
	private static final Pattern VALID_EAMIL_PATTERN = Pattern.compile(Constant.EMAIL_PATTERN);
	private static final Pattern MOBILE_PATTERN = Pattern.compile(Constant.MOBILE_PATTERN);

	private static final List<String> VALID_UPDATE_STATUS = Arrays.asList(Constant.STATUS_ACTIVE,
			Constant.STATUS_DELEATED);

	@Autowired
	private UserUtils userUtils;

	@Override
	public boolean supports(Class<?> clazz) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public void validate(Object target, Errors errors) {
		// TODO Auto-generated method stub

	}

	// customerType
	public void saveCustomerType(CustomerTypeDTO customerTypeDTO, Errors errors) {
		Long logedUserid = userUtils.getLogedInUser();
		String createdTime = DateUtils.getAsiaLocalDateTimeInCustomFormat();

		if (CustomValidator.isEmpty(customerTypeDTO.getType()))
			errors.rejectValue("type", BAD_REQUEST_ERROR_CD, "is an empty or not in valid format");

		customerTypeDTO.setStatus(Constant.STATUS_ACTIVE);
		customerTypeDTO.setCreatedDate(createdTime);
		customerTypeDTO.setCreatedBy(logedUserid);

	}

	public void getAllCustomerTypes(CustomerTypeDTO customerTypeDTO, Errors errors) {
		Long logedUserid = userUtils.getLogedInUser();
		String createdTime = DateUtils.getAsiaLocalDateTimeInCustomFormat();

		if (null == customerTypeDTO.getStatus())
			customerTypeDTO.setStatus(Constant.STATUS_ACTIVE);

		customerTypeDTO.setUpdatedBy(logedUserid);
		customerTypeDTO.setUpdatedDate(createdTime);
	}

	public void getCustomerTypeById(CustomerTypeDTO customerTypeDTO, Errors errors) {

		Long logedUserid = userUtils.getLogedInUser();
		String createdTime = DateUtils.getAsiaLocalDateTimeInCustomFormat();

		if (CustomValidator.isEmpty(customerTypeDTO.getId()))
			errors.rejectValue("id", BAD_REQUEST_ERROR_CD, "is an empty or not in valid format");

		customerTypeDTO.setUpdatedBy(logedUserid);
		customerTypeDTO.setUpdatedDate(createdTime);
	}

	public void updateCustomerType(CustomerTypeDTO customerTypeDTO, Errors errors) {
		Long logedUserid = userUtils.getLogedInUser();
		String createdTime = DateUtils.getAsiaLocalDateTimeInCustomFormat();

		if (CustomValidator.isEmpty(customerTypeDTO.getId()))
			errors.rejectValue("id", BAD_REQUEST_ERROR_CD, "is an empty or not in valid format");

		if (null != customerTypeDTO.getType() && CustomValidator.isEmpty(customerTypeDTO.getType()))
			errors.rejectValue("type", BAD_REQUEST_ERROR_CD, "is an empty or not in valid format");

		customerTypeDTO.setUpdatedBy(logedUserid);
		customerTypeDTO.setUpdatedDate(createdTime);

	}

	public void deleteCustomerType(CustomerTypeDTO customerTypeDTO, Errors errors) {
		String createdDate = DateUtils.getAsiaLocalDateTimeInCustomFormat();
		Long logedUserid = userUtils.getLogedInUser();

		if (CustomValidator.hasValidValue(customerTypeDTO.getId())) {
			errors.rejectValue("id", BAD_REQUEST_ERROR_CD, "is an empty or not in valid format");

			customerTypeDTO.setUpdatedDate(createdDate);
			customerTypeDTO.setUpdatedBy(logedUserid);
		}
	}

}
